...
class ENVIRONMENT : public RaisimGymEnv {
 ...
 public:
  ...
  explicit ENVIRONMENT(const std::string& resourceDir, const Yaml::Node& cfg, bool visualizable) :
    ...
    obDim_ = 33;
    actionDim_ = nJoints_*4; // joint pos, joint torque
    ...
    joint_normDist_ = std::normal_distribution<float>(0.f,0.05f);
    jointVel_normDist_ = std::normal_distribution<float>(0.f,0.2f);
    vel_normDist_ = std::normal_distribution<float>(0.f,0.3f);
    yaw_normDist_ = std::normal_distribution<float>(0.f,0.3f);
    joint_pos_noise_ = std::normal_distribution<float>(0.f,5*0.025);
    vel_noise_ = std::normal_distribution<float>(0.f,10*0.025);
  }
  ...
  float step(const Eigen::Ref<EigenVec>& action, int step) final {
    // position action
    ...
    if(step<50)
      rewards_.record("forwardVel",exp(-(0.1-bodyLinearVel_[0])*(0.1-bodyLinearVel_[0])));
    else if(step<110)
      rewards_.record("forwardVel",exp(-(0.2-bodyLinearVel_[0])*(0.2-bodyLinearVel_[0])));
    else if(110<=step && step<160)
      rewards_.record("forwardVel",exp(-(0.3-bodyLinearVel_[0])*(0.3-bodyLinearVel_[0])));
    else if(160<=step && step<250)
      rewards_.record("forwardVel",exp(-(0.4-bodyLinearVel_[0])*(0.4-bodyLinearVel_[0])));
    else if(250<=step && step<590)
      rewards_.record("forwardVel",exp(-(0.5-bodyLinearVel_[0])*(0.5-bodyLinearVel_[0])));
    else
      rewards_.record("forwardVel",exp(-(0.54-bodyLinearVel_[0])*(0.54-bodyLinearVel_[0])));
  ...
  void updateObservation() {
    ...
    raisim::Vec<3> gravity_ = world_->getGravity();   
    ...
    obDouble_ <<
        ...
        gc_(2),
        gravity_.e(),
        rot.e()(Eigen::seq(0,2),2);
  }

  void observe(Eigen::Ref<EigenVec> ob,bool isnoise) final {
    if(isnoise){
      ob = obDouble_.cast<float>();
      for(int i=0;i<12;i++){
        ob(i+7) += joint_pos_noise_(gen_);
      }
      for(int i=0;i<3;i++){
        ob(i+20) += vel_noise_(gen_);
      }
    }
    else{
      /// convert it to float
 ...
 private:
  ...
  std::normal_distribution<float> joint_normDist_;
  std::normal_distribution<float> jointVel_normDist_;
  std::normal_distribution<float> vel_normDist_;
  std::normal_distribution<float> yaw_normDist_;  
  std::normal_distribution<float> joint_pos_noise_;
