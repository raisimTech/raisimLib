...
from raisimGymTorch.env.bin.MPC_BC_tmp import RaisimGymEnv
from raisimGymTorch.env.bin.MPC_BC_tmp import NormalSampler
from raisimGymTorch.env.RaisimGymVecEnv import RaisimGymVecEnv as VecEnv
from raisimGymTorch.helper.raisim_gym_helper import ConfigurationSaver, load_param
import os
import math
import time
import raisimGymTorch.algo.ppo_BC_tmp.module as ppo_module
import raisimGymTorch.algo.ppo_BC_tmp.ppo_BC_tmp as PPO_BC
...
import random
from raisimGymTorch.mpc_BC_tmp import control
###################################################################
...
wandb.init(project="MPC_guide",group="1007",job_type="train",name="Train with BC loss",
            notes="Train with Bc loss"
...
joint_pos_init_obs = obs[0,8:20]
controller = control._setup_controller()
controller.reset(0.0,joint_pos_init_obs)
...
for step in range(2*n_steps):
    control_obs = env.observe(update_statistics = False, israw= True)[0]
    control._update_controller_params(controller,np.array([0.6,0.0,0.0]),0.0)
    controller.update(ctr_time_step*step,control_obs)
    mpc_act = controller.get_contact_force(control_obs) # 4 contact forces
    contact_force = mpc_act
    control_act_tmp = controller.get_control_action(contact_force,control_obs)
    ...
    control_act = np.append(np.append(pos_action,tq_action),np.append(kp_action,kd_action))
    control_act = np.tile(control_act,(num_env,1))
    mpc_data['state'].append(control_obs)
    mpc_data['action'].append(mpc_act.reshape(12))
...
for update in range(cfg['environment']['updates']):
        ...
        if cfg['environment']['render']:
            ...
            for step in range(n_steps*2):
                with torch.no_grad():
                    ...
                    simulate_control_act = np.zeros((num_env,control_act_dim))
                    ...
                    rl_action = rl_action.reshape((rl_action.shape[0],4,3))
                    for i in range(num_env):
                        ...
                        action_tmp = controller[i].get_control_action(rl_action[i],control_obs[i])
    ...
    mpc_obs = np.array(mpc_data["state"])
    mpc_obs = (mpc_obs - obs_mean)/np.sqrt(obs_var+1e-8)
    mpc_act = np.array(mpc_data["action"])
