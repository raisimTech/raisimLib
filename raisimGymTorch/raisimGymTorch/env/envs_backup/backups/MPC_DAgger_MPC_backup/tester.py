...
from raisimGymTorch.env.bin import MPC_guide_buffer_mode
...
parser.add_argument('-w', '--weight', help='trained weight path', type=str, default='/home/hyunyoungjung/MPC-RL/raisim/raisim_ws/raisimLib/raisimGymTorch/data/MPC_guide/2022-09-25-20-36-04/full_800.pt')
...
################# my wandb #####################################
wandb.init(project="MPC_guide",group="0926",job_type="test",name="",notes="train with renewed data"
            ,config=cfg,save_code=True, mode = 'disabled')
################################################################

env = VecEnv(MPC_guide_buffer_mode.RaisimGymEnv(home_path + "/rsc", dump(cfg['environment'], Dumper=RoundTripDumper)), cfg['environment'])
...
# simulation
    ...
    # load sampler
    loaded_sampler = ppo_module.MultivariateGaussianDiagonalCovariance(act_dim,env.num_envs,1.0,MPC_guide_buffer_mode.NormalSampler(act_dim),cfg['seed'])
    ...
    for step in range(total_steps*2):
        ...
        ## sampling version
        action_mean = loaded_graph.architecture(torch.from_numpy(obs).cpu())
        action_ll, _ = loaded_sampler.sample(action_mean.detach().numpy())
