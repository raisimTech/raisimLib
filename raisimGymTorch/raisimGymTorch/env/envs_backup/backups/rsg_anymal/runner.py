...
wandb.init(project="0812",group="code_test",job_type="test",name="vectorized_env_modification2",notes="Different seeds to different thread."
...
wandb.save(saver.data_dir+"/Environment.hpp")
wandb.save(saver.data_dir+"/cfg.yaml")
#########################################################################################

############################### my edit. comment original code ###############################################
...
for update in range(1000000):
    ...
    if update % cfg['environment']['eval_every_n'] == 0:
        ...
        ######################## my record to wandb ################################################
        # while(not os.path.isfile("/home/hyunyoungjung/MPC-RL/raisim/raisim_ws/raisimLib/raisimUnity/linux/Screenshot/"+vid_name)):
        #     time.sleep(0.1)
        # time.sleep(15)
        # wandb.log({"video": wandb.Video("/home/hyunyoungjung/MPC-RL/raisim/raisim_ws/raisimLib/raisimUnity/linux/Screenshot/"+vid_name, caption=str(update), fps=4, format="mp4")})
        ############################################################################################
    ...
    for step in range(n_steps):
        ...
        velocities["forward"] += np.sum(obs_raw[:,16])
        velocities["total"] += np.sum(np.linalg.norm(obs_raw[:,16:19],axis=1))
        velocities["yawing"] += abs(obs_raw[0][21])

        tmp_rewardinfo = env.getRewardinfo()
        for i in range(len(tmp_rewardinfo)):
            for key in tmp_rewardinfo[i]:
                if key in rewardinfo.keys():
                    rewardinfo[key] += tmp_rewardinfo[i][key]
                else:
                    rewardinfo[key] = tmp_rewardinfo[i][key]
        # ##################################################################################
    ...
    ####my code####################################################################################################
    #record reward and velocity
    if update%10 == 0:
        wandb.log({"average_reward":average_ll_performance})
        for key in rewardinfo:
            wandb.log({key:rewardinfo[key] / total_steps})
        
        wandb.log({"forward_velocity": velocities['forward'] / total_steps})
        wandb.log({"total_velocity": velocities['total'] / total_steps })
        wandb.log({"yawing_velocity": velocities['yawing'] / total_steps})
    ###############################################################################################################
    actor.update()
