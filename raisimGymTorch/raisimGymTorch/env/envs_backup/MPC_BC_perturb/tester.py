...
from raisimGymTorch.env.bin import MPC_BC_perturb
...
parser.add_argument('-w', '--weight', help='trained weight path', type=str, \
    default='/home/hyunyoungjung/MPC-RL/raisim/raisim_ws/raisimLib/raisimGymTorch/data/MPC_BC_perturb/2022-10-18-05-12-52/full_1000.pt')
...
################# my wandb #####################################
wandb.init(project="MPC_guide",group="1017",job_type="test",name="MPC BC perturbed",notes="BC loss with perturbed data"
            ,config=cfg,save_code=True, mode = cfg['wandb_mode'])
################################################################

env = VecEnv(MPC_BC_perturb.RaisimGymEnv(home_path + "/rsc", dump(cfg['environment'], Dumper=RoundTripDumper)), cfg['environment'])
...
# simulation
    ...
    # load sampler
    loaded_sampler = ppo_module.MultivariateGaussianDiagonalCovariance(rl_act_dim,env.num_envs,1.0,MPC_BC_perturb.NormalSampler(rl_act_dim),cfg['seed'])
