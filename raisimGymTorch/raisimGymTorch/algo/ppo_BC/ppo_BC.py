...
class PPO:
    ...
    def add_mpc_data(self,mpc_obs,mpc_act):
        self.storage.add_mpc_data(mpc_obs,mpc_act)
        
    def update(self, actor_obs, value_obs, log_this_iteration, w_im, w_rl, update):
        ...
        mean_value_loss, mean_surrogate_loss, mean_imiation_loss, infos = \
                                    self._train_step(log_this_iteration,w_im,w_rl)
        self.storage.clear()
    ...
    def log(self, variables):
        ...
        wandb.log({"value_loss":variables['mean_value_loss'],"imitation_loss":variables['mean_imitation_loss'],\
    ...
    def _train_step(self, log_this_iteration,w_im,w_rl):
        mean_value_loss = 0
        mean_surrogate_loss = 0
        mean_imitation_loss = 0

        for epoch in range(self.num_learning_epochs):
            for actor_obs_batch, critic_obs_batch, actions_batch, old_sigma_batch, old_mu_batch, current_values_batch, advantages_batch, \
                returns_batch, old_actions_log_prob_batch, mpc_obs_batch, mpc_act_batch in self.batch_sampler(self.num_mini_batches):
                ...
                # imitation loss
                imitation_loss = 0
                act_BC = self.actor.evaluate_BC(mpc_obs_batch)
                imitation_loss = (act_BC-mpc_act_batch).pow(2).mean()
                loss = w_rl * (surrogate_loss + self.value_loss_coef * value_loss - self.entropy_coef * entropy_batch.mean()) + \
                    w_im * imitation_loss
                # Gradient step
        ...
        return mean_value_loss, mean_surrogate_loss, mean_imitation_loss, locals()