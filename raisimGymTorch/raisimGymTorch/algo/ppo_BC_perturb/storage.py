...
class RolloutStorage:
    ...
    def mini_batch_generator_inorder(self, num_mini_batches,mpc_obs=None,mpc_acts=None, mpc_batch_size=None):
        batch_size = self.num_envs * self.num_transitions_per_env
        mini_batch_size = batch_size // num_mini_batches
        assert mpc_obs.shape[0] == mpc_batch_size, "mpc data size is wrong!"
        self.mpc_obs_tc = torch.from_numpy(mpc_obs).to(self.device)
        self.mpc_acts_tc = torch.from_numpy(mpc_acts).to(self.device)
        mpc_mini_batch_size = mpc_batch_size // num_mini_batches            
        ...
        for batch_id in range(num_mini_batches):
            ...
            self.mpc_obs_tc.view(-1,self.mpc_obs_tc.size()[-1])[batch_id*mpc_mini_batch_size:(batch_id+1)*mpc_mini_batch_size], \
            self.mpc_acts_tc.view(-1,self.mpc_acts_tc.size()[-1])[batch_id*mpc_mini_batch_size:(batch_id+1)*mpc_mini_batch_size]