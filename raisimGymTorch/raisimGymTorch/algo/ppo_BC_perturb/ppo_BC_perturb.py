...
class PPO:
    ...
    def update(self, actor_obs, value_obs, log_this_iteration, w_im_arg, w_rl_arg, update, mpc_obs=None, mpc_acts=None):
        ...
        mean_value_loss, mean_surrogate_loss, mean_imiation_loss, infos = \
                                    self._train_step(log_this_iteration,w_im_arg,w_rl_arg,mpc_obs,mpc_acts)
        self.storage.clear()
    ...
    def _train_step(self, log_this_iteration,w_im_arg,w_rl_arg,mpc_obs=None,mpc_acts=None):
        mean_value_loss = 0
        mean_surrogate_loss = 0
        mean_imitation_loss = 0
        mpc_batch_size = mpc_obs.shape[0]
        for epoch in range(self.num_learning_epochs):
            for actor_obs_batch, critic_obs_batch, actions_batch, old_sigma_batch, old_mu_batch, current_values_batch, advantages_batch, returns_batch, old_actions_log_prob_batch \
                    , mpc_obs_batch, mpc_acts_batch in \
                    self.batch_sampler(self.num_mini_batches,mpc_obs,mpc_acts,mpc_batch_size):
                ...
                imitation_loss = (act_BC-mpc_acts_batch).pow(2).mean()
