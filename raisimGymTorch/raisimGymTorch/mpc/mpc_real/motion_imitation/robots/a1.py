...
class A1(minitaur.Minitaur):
  ...
  def __init__(
    ...
    super(A1, self).__init__(
        ...
        raisim_robot_name=raisim_robot_name,
        raisim_world=raisim_world,
        time_step=time_step,
  ...
  def _LoadRobotURDF(self):
    self.quadruped = self._raisim_world.getObjList().index(self._raisim_world.getObject(self._raisim_robot_name))
    self._raisim_robot.setBasePos(self._GetDefaultInitPosition())
    self._raisim_robot.setBaseOrientation(self._GetDefaultInitOrientation())
  ...
  def GetFootContacts(self):
    ...
    all_contacts = self._raisim_robot.getContacts()
    for contact in all_contacts:
        # Ignore self contacts
        if contact.isSelfCollision():
            continue
        try:
            toe_link_index = self._foot_link_ids.index(contact.getlocalBodyIndex())
  ...
  def ResetPose(self, add_constraint):
    del add_constraint
    gv = np.zeros(self._raisim_robot.getDOF())
    gc = []
    gc.extend(self._GetDefaultInitPosition())
    tmp_orient = self._GetDefaultInitOrientation()
    gc.append(tmp_orient[-1])
    gc.extend(tmp_orient[0:-1])
    ...
    self._raisim_robot.setState(gc,gv)
  ...
  def _BuildUrdfIds(self):
    ...
    """
    self._foot_link_ids = []
    self._foot_link_ids.append(self._raisim_robot.getBodyIdx("FR_calf"))
    self._foot_link_ids.append(self._raisim_robot.getBodyIdx("FL_calf"))
    self._foot_link_ids.append(self._raisim_robot.getBodyIdx("RR_calf"))
    self._foot_link_ids.append(self._raisim_robot.getBodyIdx("RL_calf"))
