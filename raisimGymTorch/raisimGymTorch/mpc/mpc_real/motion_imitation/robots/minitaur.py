...
class Minitaur(object):
  ...
  def __init__(self,
    ...
    self._raisim_robot_name = raisim_robot_name
    self._raisim_world = raisim_world
    self._raisim_robot = self._raisim_world.getObject(self._raisim_robot_name)
  ...
  def _BuildJointNameToIdDict(self):
    ...
    num_joints = self._raisim_robot.getDOF()-6
    self._joint_name_to_id = {}
    for i in range(num_joints):
      # joint_info = self._pybullet_client.getJointInfo(self.quadruped, i)
      # self._joint_name_to_id[joint_info[1].decode("UTF-8")] = joint_info[0]
      self._joint_name_to_id[joint_names[i]] = self._raisim_robot.getFrameIdxByName(joint_names[i])
  ...
  def _RemoveDefaultJointDamping(self):
    ...
    #                                        linearDamping=0,
    #                                        angularDamping=0)
    self._raisim_robot.setJointDamping(np.zeros(self._raisim_robot.getDOF()))
  ...
  def GetBaseVelocity(self):
    ...
    velocity = self._raisim_robot.getGeneralizedVelocity()[:3]
    return velocity
  ...
  def GetBaseOrientation(self):
    ...
    # return self._raisim_robot.getBaseOrientation()
  ...
  def GetTrueBaseRollPitchYawRate(self):
    ...
    angular_velocity = self._raisim_robot.getGeneralizedVelocity()[3:6]
    orientation = self.GetTrueBaseOrientation()
  ...
  def ApplyAction(self, motor_commands, motor_control_mode):
    ...
    desired_motor_angles = np.zeros(7)
    desired_motor_velocities = np.zeros(6)
    kp = np.zeros(6)
    kd = np.zeros(6)
    additional_torques = np.zeros(6)
    if motor_control_mode is robot_config.MotorControlMode.POSITION:
      kp = np.append(kp,self._motor_kps)
      kd = np.append(kd,self._motor_kds)
      desired_motor_angles = np.append(desired_motor_angles,motor_commands)
      desired_motor_velocities = np.append(desired_motor_velocities,np.full(12, 0))
      additional_torques = np.append(additional_torques,np.full(12, 0))
    else:
      kp = np.append(kp,motor_commands[POSITION_GAIN_INDEX::MOTOR_COMMAND_DIMENSION])
      kd = np.append(kd,motor_commands[VELOCITY_GAIN_INDEX::MOTOR_COMMAND_DIMENSION])
      desired_motor_angles = np.append(desired_motor_angles,motor_commands[
          POSITION_INDEX::MOTOR_COMMAND_DIMENSION])
      desired_motor_velocities = np.append(desired_motor_velocities,motor_commands[
          VELOCITY_INDEX::MOTOR_COMMAND_DIMENSION])
      additional_torques = np.append(additional_torques,motor_commands[TORQUE_INDEX::MOTOR_COMMAND_DIMENSION])

    self._raisim_robot.setPdGains(kp,kd)
    self._raisim_robot.setPdTarget(desired_motor_angles,desired_motor_velocities)
    self._raisim_robot.setGeneralizedForce(additional_torques)
  ...
  def ReceiveObservation(self):
    ...
    gc_state = self._raisim_robot.getGeneralizedCoordinate()[7:]
    gv_state = self._raisim_robot.getGeneralizedVelocity()[6:]
    self._joint_states = []
    for i in range(len(gc_state)):
      self._joint_states.append([gc_state[i],gv_state[i]])
    # self._base_position, orientation = (
    #     self._pybullet_client.getBasePositionAndOrientation(self.quadruped))
    self._base_position, orientation = (self._raisim_robot.getGeneralizedCoordinate()[:3],
                                        self._raisim_robot.getBaseOrientation())
    ...
    self._base_orientation.extend(orientation[1:])
    self._base_orientation.append(orientation[0])
