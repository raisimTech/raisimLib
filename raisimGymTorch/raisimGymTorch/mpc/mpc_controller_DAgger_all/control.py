from raisimGymTorch.mpc.mpc_controller_DAgger_all import com_velocity_estimator
from raisimGymTorch.mpc.mpc_controller_DAgger_all import gait_generator as gait_generator_lib
from raisimGymTorch.mpc.mpc_controller_DAgger_all import locomotion_controller
from raisimGymTorch.mpc.mpc_controller_DAgger_all import openloop_gait_generator
from raisimGymTorch.mpc.mpc_controller_DAgger_all import raibert_swing_leg_controller
from raisimGymTorch.mpc.mpc_controller_DAgger_all import torque_stance_leg_controller_quadprog as torque_stance_leg_controller
...
class Controller(object):
    ...
    def __init__(
        ...
        self.controller = []
        self.motor_kd = a1.motor_kd
        self.motor_kp = a1.motor_kp
        self.time_since_reset = np.zeros(self.num_env)
        self.reset_time = np.zeros(self.num_env)
    ...
    def reset(self,reset_time,init_joint,env_num):
        ...
        self.time_since_reset[env_num] = 0.0
        self.reset_time[env_num] = reset_time
    ...
    def get_mpc_contact_force(self, ctrl_obs):
        contact_forces = []
        for i in range(self.num_env):
            indiv_obs = {'bodyOrientation': ctrl_obs['bodyOrientation'][i,:], 'footContacts':ctrl_obs['footContacts'][i,:],\
                        'jointPos':ctrl_obs['jointPos'][i,:],'baseLinVel':ctrl_obs['baseLinVel'][i,:],'baseAngVel':ctrl_obs['baseAngVel'][i,:]}
    ...
    def control_with_conactForce(self,action,ctrl_obs,step):
        ...
        self.time_since_reset = self.ctrl_time_step*step-self.reset_time
        for i in range(self.num_env):
            indiv_obs = {'bodyOrientation': ctrl_obs['bodyOrientation'][i,:], 'footContacts':ctrl_obs['footContacts'][i,:],\
                        'jointPos':ctrl_obs['jointPos'][i,:],'baseLinVel':ctrl_obs['baseLinVel'][i,:],'baseAngVel':ctrl_obs['baseAngVel'][i,:]}
