...
from raisimGymTorch.mpc_BC_baseline.mpc_controller import gait_generator as gait_generator_lib
from raisimGymTorch.mpc_BC_baseline.mpc_controller import leg_controller
from raisimGymTorch.mpc_BC_baseline import robot_helper
...
class RaibertSwingLegController(leg_controller.LegController):
  ...
  def __init__(
    ...
    self.reset(0,robot_helper.INITIAL_POS)

  def reset(self, current_time: float, joint_pos) -> None:
    ...
    self._phase_switch_foot_local_position = robot_helper.GetFootPositionsInBaseFrame(joint_pos)
    self._joint_angles = {}

  def update(self, current_time: float, joint_pos) -> None:
  ...
  def get_action(self, yaw_rate) -> Mapping[Any, Any]:
    ...
    for leg_id, leg_state in enumerate(self._gait_generator.leg_state):
      ...
      joint_ids, joint_angles = robot_helper.ComputeMotorAnglesFromFootLocalPosition(leg_id, foot_position)
      # Update the stored joint angles as needed.
      for joint_id, joint_angle in zip(joint_ids, joint_angles):
        joint_angle_norm = (joint_angle - robot_helper.INITIAL_POS[joint_id])/0.3
        self._joint_angles[joint_id] = (joint_angle, leg_id)

    action = {}
    kps = robot_helper.motor_kp
    kds = robot_helper.motor_kd
