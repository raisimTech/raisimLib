...
from raisimGymTorch.mpc_BC_baseline.mpc_controller import qp_torque_optimizer
...
class TorqueStanceLegController(leg_controller.LegController):
  ...
  def _estimate_robot_height(self, contacts, orientation, joint_pos):
    if np.sum(contacts) == 0:
      ...
      rot_mat = robot_helper.pybullet.getMatrixFromQuaternion(base_orientation)
      rot_mat = np.array(rot_mat).reshape((3, 3))

      
      foot_positions = robot_helper.GetFootPositionsInBaseFrame(joint_pos)
  ...
  def get_action(self,orientation, roll_pitch_yaw_rate,joint_pos):
    ...
    contacts = np.array(
        ...
        dtype=np.int32)
    orientation = robot_helper.get_pybullet_quat(orientation)
    ...
    robot_com_roll_pitch_yaw = np.array(robot_helper.pybullet.getEulerFromQuaternion(orientation))
    ...
    for leg_id, force in enumerate(contact_forces):
      motor_torques = robot_helper.MapContactForceToJointTorques(leg_id, force, joint_pos)
