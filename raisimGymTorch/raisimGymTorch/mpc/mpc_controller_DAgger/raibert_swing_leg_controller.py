...
from raisimGymTorch.mpc.mpc_controller_DAgger import gait_generator as gait_generator_lib
from raisimGymTorch.mpc.mpc_controller_DAgger import leg_controller
from raisimGymTorch.mpc.mpc_controller_DAgger import a1
...
class RaibertSwingLegController(leg_controller.LegController):
  ...
  def reset(self, current_time: float, joint_pos) -> None:
    ...
    self._phase_switch_foot_local_position = a1.GetFootPositionsInBaseFrame(joint_pos)
    self._joint_angles = {}
    ...
    for leg_id, state in enumerate(new_leg_state):
      if (state == gait_generator_lib.LegState.SWING
        ...
        self._phase_switch_foot_local_position[leg_id] = \
          (a1.GetFootPositionsInBaseFrame(joint_pos)[leg_id])
  ...
  def get_action(self, yaw_rate) -> Mapping[Any, Any]:
    ...
    yaw_dot = yaw_rate
    hip_positions = a1._DEFAULT_HIP_POSITIONS
    ...
    for leg_id, leg_state in enumerate(self._gait_generator.leg_state):
      ...
      joint_ids, joint_angles = a1.ComputeMotorAnglesFromFootLocalPosition(leg_id, foot_position)
      # Update the stored joint angles as needed.
      for joint_id, joint_angle in zip(joint_ids, joint_angles):
        joint_angle_norm = (joint_angle - a1.INITIAL_POS[joint_id])/0.3
